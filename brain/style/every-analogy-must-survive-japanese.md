---
id: every-analogy-must-survive-japanese
layer: style
status: draft
confidence: high
decay: durable
...
**Applied to the house analogies.**

- **Bank teller / vault** — a concrete, universal image. Survives.
- **Renovating a building vs editing a document** — physical and universal. Survives.
- Rejected pattern: any analogy resting on an English phrase that is funny or memorable only in English, or on a sport, snack, or show that does not read the same abroad.

**Interaction with the analogy record.** This rule is one of the gates in `our-analogies-chosen-and-rejected`: an analogy passes only if it carries the boundary *and* survives Japanese. Record the rejection reason when an otherwise-good analogy fails here, so it is not proposed again next course.
