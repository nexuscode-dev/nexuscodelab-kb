...
*Grounding: the **Platform Fit** section is read from `myanlearn@origin/develop` @ `3d34a4e` via the two L4 notes
and `tests/platform-verification-2026-08-25.md`. Everything under **Course Structure** and **Sample Lesson** is a
**proposal with no knowledge behind it yet** — the vault holds zero domain notes and zero sources on design or
front-end. Course order is not assumed here; this is a candidate, not "course 2".*
...
mode T2 exists to catch. One L2 slot has to be spent before any lesson is generated.
...
— the platform cannot assess at all. See Platform Fit and Open Question 1. That is not a reason to drop the
course; it is the decision that has to be made before it is built.
...
**None generated.** Course 1's Lesson 13 sample exists because its domain note existed first; here no domain note
does, so generating a sample now would test the model's knowledge of CSS, not the vault's.

The lesson proposed *as* the sample, once its note is written, is the structural twin of Lesson 13:
...
- **Quizzes are narrow.** Single-answer multiple choice only; no true/false, no multi-select, and **no field to
  explain a wrong answer**, so the wrong options carry all the teaching. Quizzes are timed, every question must be
  answered or the submission is rejected, and **the pass mark is a raw count of correct questions, not a
  percentage** — 4 questions pass 3 per section quiz, 8 pass 6 for the cumulative.
...
**Ready** — the platform read: what a lecture can hold, what a quiz can assess, and the proof that visual labs
cannot work. Course 1's notes that this course would build on.

**Pending** — a decision on Open Question 1, without which the course cannot be scoped; a staging test of code-
sample escaping; the L2 persona note; the missing L4 note that §12 decision 7 already specified and nobody wrote.

**Not started** — the charter, every domain note (**zero exist**), every source record (**zero exist**), and all
20 lessons. Nothing is written and nothing is published.

## Open Questions

1. **The assessment fork, and this proposal is its trigger.** KB_DESIGN_PROPOSAL §12 decision 7 deferred the
   CSS-lab question with a named forcing event: *"the first course charter proposing a lesson whose assessed
   outcome is 'the learner produces visual output'."* **This is that proposal**, so decision 7 reopens on
   approval. The three branches are unchanged: teach only what JavaScript can grade, extend the grader with a real
   language map, or design the course around having no working labs (the recommendation above). Its own note,
   `brain/platform/css-labs-cannot-be-authored.md`, was specified by that decision and **does not exist** — so
   retrieval question 4 currently has nowhere to land.
2. **Can code samples containing tags be published safely?** One staging test — publish a lecture with an escaped
   `<div>` sample and read it back — settles whether this course is authorable at all. It blocks S1 and S2
   entirely, so it should run before the charter, not after.
3. **Whose course is it?** A persona note for the career-switcher, or an explicit decision that this course is
   written for the student. Either way it costs one L2 slot, and no lesson should be generated before it exists.
4. **The note budget does not fit two courses.** §7 caps L3 at 12–15 notes for v1, four are written, and course 1
   alone lists 17 candidates against that cap. A second course's domain notes cannot fit without a cap decision at
   the v1 review — caps cannot be raised mid-build. *Also worth settling in the same pass: the backlog files
   currently print different caps from §7 (L2 3–4 vs 6–8, L3 15–20 vs 12–15), and any budget argument about this
   course depends on which is authoritative.*
